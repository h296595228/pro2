...
function doPost($id){
	...
	$input = [
		...
		'shows' => I('save', 'post', 'bool') ? 'no' : 'yes', //是否发布
	...
	if($id){
		//修改数据
		$input['id'] = $id;
		db_query('UPDATE `cms_article` SET `cid`=?,`title`=?,`author`=?,`shows`=?,`content`=?,`keywords`=?,`description`=?,`thumb`=? WHERE `id`=?', 'isssssssi', $input);
		display([true, '修改成功。<a href="cp_article.php">返回列表</a>'], $id);
	}else{
		//添加数据
		$input['time'] = date('Y-m-d H:i:s');
                $add_id = db_exec(DB_LASTID, 'INSERT INTO `cms_article` (`cid`,`title`,`author`,`shows`,`content`,`keywords`,`description`,`thumb`,`time`) VALUES(?,?,?,?,?,?,?,?,?)', 'issssssss', $input);
...
function display($msg=null, $id=0, $data=[]){
	if($id && empty($data)){
		//根据ID查出原有的记录
		if(!$data = db_fetch(DB_ROW, 'SELECT `cid`,`title`,`author`,`shows`,`content`,`keywords`,`description`,`thumb` FROM `cms_article` WHERE `id`=?', 'i', $id)){
			E('数据不存在！');
		}
	}else{
		//合并模板变量（初始值）
