...
function display($msg=null, $page_size=4){
        //获取列表参数
	...
	$order_arr = [
		...
		'show-desc' => ['name'=>'发布状态', 'sql'=>'a.`shows` DESC']
	];
        $sql_order = ' ORDER BY ';
	...
	//查询列表
	$data = db_fetch(DB_ALL, 'SELECT a.`id`,a.`cid`,a.`title`,a.`author`,a.`shows`,a.`time`,c.`name` AS cname'
	...
	//		ORDER BY a.`id` DESC');
        //获取分页HTML
	$page_html = page_html($page_total, $page, $page_size);
	//判断页码和结果
	if(!$data){
		if($page > 1){
