...
function module_article_show($id){
	return db_fetch(DB_ROW, 'SELECT `cid`,`title`,`time`,`views`,`keywords`,`thumb`,`author`,`description`,`content` FROM `cms_article` WHERE `id`=? AND `shows`=\'yes\'', 'i', $id);	
}
...
function module_hot($limit=10){
	return db_fetch(DB_ALL, "SELECT `id`,`title` FROM `cms_article` WHERE `shows`='yes' ORDER BY `views` DESC LIMIT 0,$limit");
}
...
function module_history($current=false, $limit=10){
	...
	if(!empty($result)){
		...
		return db_fetch(DB_ALL, "SELECT `id`,`title` FROM `cms_article` WHERE `id` IN($sql_in) AND `shows`='yes' ORDER BY FIELD(`id`,$sql_in)");
	}
	return [];
}